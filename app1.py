...
def predict_attrition(Age,OverTime_Yes,MonthlyIncome,TotalWorkingYears,DailyRate,HourlyRate,MonthlyRate,YearsAtCompany,PerformanceRating,):
    
   
    prediction=classifier.predict([[Age,OverTime_Yes,MonthlyIncome,TotalWorkingYears,DailyRate,HourlyRate,MonthlyRate,YearsAtCompany,PerformanceRating]])
    print(prediction)
    return prediction
...
def main():
    ...
    OverTime_Yes = st.text_input("OverTimeYes",placeholder="If the Employee Works Overtime (1 or 0): ")
    ...
    PerformanceRating = st.text_input("PerformanceRating", placeholder = "Enter the performance rating for the Employee : ")

    currentManager = st.text_input("currentManager", placeholder = "")
    result=""
